...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
	...
	exclude-result-prefixes="xalan encoder i18n dri mets dim xlink xsl" version="1.0">
	...
	<xsl:template name="itemSummaryView-DIM-fields">
		...
		<xsl:choose>
			<!-- Want a thumbnail preview? see dri2html/DIM-Handler.xsl for source. -->
			...
			<xsl:when test="$clause = 1">
				...
				<!-- Update $clause, $phase, and recurse -->
			...
			<!-- Citation row -->
			<xsl:when test="$clause = 3 and (dim:field[@element='identifier' and @qualifier='citation'])">
				<tr class="ds-table-row {$phase}">
					<td>
						<span class="bold">
							<i18n:text>xmlui.ETCRiverRun.METS-1.0.item-citation</i18n:text>: 
						</span>
					</td>
					<td>
						...
						<xsl:for-each select="dim:field[@element='identifier' and @qualifier='citation']">
							<xsl:copy-of select="./node()"/>
							<xsl:if
								test="count(following-sibling::dim:field[@element='identifier' and @qualifier='citation']) != 0">
								<hr class="metadata-seperator"/>
							</xsl:if>
						</xsl:for-each>
						<xsl:if test="count(dim:field[@element='identifier' and @qualifier='citation']) &gt; 1">
			...
			<xsl:when test="$clause = 5 and (dim:field[@element='identifier' and @qualifier='doi'])">
				
				<!-- http://dx.doi.org/10.1080/002689799163172 -->
				<tr class="ds-table-row {$phase}">
					<td>
						<span class="bold">
							<i18n:text>xmlui.ETCRiverRun.METS-1.0.item-doi</i18n:text>: 
						</span>
					</td>
					<td>
						...
						<xsl:for-each select="dim:field[@element='identifier' and @qualifier='doi']">
							...
							<xsl:if
								test="count(following-sibling::dim:field[@element='identifier' and @qualifier='doi']) != 0">
								<hr class="metadata-seperator"/>
							</xsl:if>
						</xsl:for-each>
						<xsl:if test="count(dim:field[@element='identifier' and @qualifier='doi']) &gt; 1">
			...
			<!-- Other online version(s), identifier.uri -->
			<xsl:when test="$clause = 6 and (dim:field[@element='identifier' and @qualifier='uri'])">
				<tr class="ds-table-row {$phase}">
					<td>
						<span class="bold">
							<i18n:text>xmlui.ETCRiverRun.METS-1.0.item-uri</i18n:text>: </span>
					</td>
					<td>
			...
			<xsl:when test="$clause = 7">
				<xsl:variable name="handleURL" select="concat('http://hdl.handle.net/',substring-after(/mets:METS/@ID, ':'))"/>
				<tr class="ds-table-row {$phase}">
					<td>
						<span class="bold">
							<i18n:text>xmlui.ETCRiverRun.METS-1.0.item-handle</i18n:text>: </span>
					</td>
					<td>
						<a>
							<xsl:attribute name="href">
			...
			<!-- date.issued row -->
			<xsl:when test="$clause = 9 and (dim:field[@element='date' and @qualifier='issued'])">
				<tr class="ds-table-row {$phase}">
					<td>
						<span class="bold">
